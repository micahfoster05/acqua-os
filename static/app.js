// ============================================
// Acqua OS UI — Main Application Orchestrator
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Acqua OS Chat';
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('acqua-os-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'acqua-os-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Acqua OS...');
      }
  ...
  const UI_VIS_KEY = 'acqua-os-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'acqua-os-toolbar-visibility';
    try {
...
function startAcqua OSApp() {
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('acqua-os-doc-open-' + _curSession) === '1') {
