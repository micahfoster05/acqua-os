...
const _ACTION_RECENTS_KEY = 'acqua-os-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'acqua-os-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Acqua OS';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Acqua OS' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
