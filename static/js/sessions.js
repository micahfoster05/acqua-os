...
let _sortMode = Storage.get('acqua-os-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Acqua OS Chat';
...
const FOLDER_STATE_KEY = 'acqua-os-folder-state';
const FOLDER_ORDER_KEY = 'acqua-os-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Acqua OS kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Acqua OS Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('acqua-os-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('acqua-os-session-sort', mode);
  else Storage.remove('acqua-os-session-sort');
  renderSessionList();
}
